...
| \`results\` | FileResult[] | 파일 처리 결과 (성공/실패) |
...
- 파일 처리 결과 확인: \`useMDSFileUploadState(controller, 'results')\`
...
export const FileResults: StoryObj<UseMDSFileUploaderOptions> = {
  parameters: {
    docs: {
      description: {
        story: `\`useMDSFileUploadState(controller, 'results')\`로 파일 처리 결과를 구독합니다.

**results vs errors의 차이:**
- \`errors\`: 유효성 검증 실패(파일 타입, 크기 등)와 업로드 실패 에러만 포함
- \`results\`: 성공/실패 **모든** 파일의 처리 결과를 포함 (성공 시 url, 실패 시 error 정보)

업로드 완료 후 각 파일의 결과를 확인하거나, 성공한 파일의 URL을 수집할 때 유용합니다.

**테스트 방법:**
- 이미지 파일 업로드 → 성공 결과 (status: 'success', url 포함)
- 이미지가 아닌 파일 업로드 → 실패 결과 (status: 'error', INVALID_FILE_TYPE)
- 이미지 + 비이미지 파일을 함께 업로드 → 성공/실패 결과 혼합`,
      },
    },
  },
  args: {
  ...
  render: function Render(props) {
    ...
    const { file, progress, controller, error } = useMDSFileUploader({
      ...props,
      uploadUrl: mockGetUploadUrl,
    });
    const { value, open, reset } = file;
    const { isUploading } = progress;
    const results = useMDSFileUploadState(controller, 'results');
      ...
      <Wrapper>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {(results.length > 0 || value.length > 0) && (
            <MDSPlainButton startIcon={<MDSIcon.Reset />} onClick={handleReset}>
              Reset
            </MDSPlainButton>
          )}
          {value.map((_, index) => (
        ...
        <div style={{ marginTop: '12px' }}>
          <span style={{ color: '#666', fontSize: '14px' }}>결과 수: {results.length}개</span>
        </div>
        <div style={{ marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {results.map((result, resultIndex) => (
            <MDSMessageBox
              ...
              title={`[${result.status.toUpperCase()}] ${result.file.name}`}
              message={
                result.status === 'success' ? `URL: ${result.url}` : `[${result.error?.code}] ${result.error?.message}`
              }
            />
          ))}
        </div>
